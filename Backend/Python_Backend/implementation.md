...
## Data Models and Input Handling

### Schema Architecture

Our system uses three key files to handle data validation and processing:

1. **common/mcp_schemas.py** (Message Protocol Models)
2. **common/schemas.py** (Domain Models)
3. **orchestration_agent/core/input_handler.py** (Validation Logic)

### 1. MCP/ACL Schema (mcp_schemas.py)

This file defines the Model Context Protocol (MCP) and Agent Communication Language (ACL) structure that the LLM layer uses to communicate with our system:
...
Example LLM output in this format:
```json
{
    "mcp": {
        "user_id": "123",
...
### 2. Domain Models (schemas.py)

This file contains the domain-specific models used by individual agents:
...
These models define:
- The expected input format for each agent
- The structure of responses from each agent
- Optional fields and error handling

### 3. Input Handler (input_handler.py)

This component validates and processes incoming MCP/ACL messages:
...
The Input Handler performs:
1. Structural validation (using Pydantic models)
2. Agent existence verification
3. Action validation for each agent
4. Required parameter checking
5. Error handling and reporting

## LLM Service Implementation

### Overview of LLMService

The `LLMService` class is responsible for handling interactions with the Gemini Pro model through Vertex AI and LangChain. It processes user messages, generates MCP/ACL structures, and ensures robust error handling. The service is implemented in `orchestration_agent/services/llm_service.py`.
...
self.prompt = PromptTemplate(
    ...
    1. MCP context (user_id, session_id, metadata)
...
The service implements a robust multi-layer error handling approach:

1. **Primary Try-Catch Block**
   - Handles LLM-related errors (API failures, connection issues)
   - Catches any errors during the LangChain chain execution

2. **Secondary Try-Catch Block**
   - Handles JSON parsing and validation errors
   - Attempts to recover by creating a default structure if parsing fails

3. **Helper Methods for Error Recovery**

a. **Default MCP/ACL Creation**
```python
...
The service enriches all MCP/ACL structures with context:
...
# The result will be a validated MCP/ACL structure with:
# - Proper MCP context (user_id, session_id, timestamp)
# - Relevant ACL tasks for disease prediction and patient journey
# - Error handling if something goes wrong
```

### Data Flow Example

1. **LLM Output → Input Handler**
   ```json
   {
       "mcp": {"user_id": "123", "session_id": "abc"},
       "acl": [
           {
               "agent": "patient_journey",
               "action": "track_journey",
...
2. **Input Handler → Task Planner**
   ```python
   validated_input = MCPACL(
       mcp=MCPContext(user_id="123", session_id="abc"),
       acl=[ACLTask(agent="patient_journey", action="track_journey", params={"patient_id": "123"})]
   )
   ```

3. **Task Planner → Agent**
   ```python
   request = PatientJourneyRequest(
       patient_id="123",
       action="track_journey",
       params={}
   )
...
4. **Agent → Response**
   ```python
   response = PatientJourneyResponse(
       visits=[{"date": "2025-09-01", "location": "General Hospital"}],
       error=None
   )
   ```
...
1. **Missing Required Parameter**
   ```json
   {
       "agent": "patient_journey",
       "action": "track_journey",
       "params": {}  // Missing patient_id
   }
   ```
   Error: "Missing required parameters for patient_journey.track_journey: patient_id"
...
2. **Invalid Action**
   ...
   ```
   Error: "Unsupported action 'invalid_action' for agent patient_journey"
