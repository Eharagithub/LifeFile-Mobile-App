...
class ChatService {
    ...
    async processHealthQuery(userMessage: string): Promise<string> {
        try {
            ...
            // Prepare payload for /chat_orchestrate
            const payload = {
                prompt: userMessage,
                user_id: 'testuser', // You can replace with actual user id
                session_id: 'session1', // You can replace with actual session id
                workflow: 'disease_prediction'
            };
            const backendUrl = Constants?.expoConfig?.extra?.backendUrl || 'http://10.146.44.156:8000';
            const fullUrl = `${backendUrl}/chat_orchestrate`;
            ...
            if (data.dispatch_results && data.dispatch_results.length > 0) {
                const result = data.dispatch_results[0];
                if (result.error) {
